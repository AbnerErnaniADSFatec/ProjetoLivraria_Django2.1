...
def livro_nome_registrado(request):
    ...
    if data['is_taken']:
        data['message'] = 'O Livro já está cadastrado'
    return JsonResponse(data)
...
def editora_nome_registrado(request):
    ...
    if data['is_taken']:
        data['message'] = 'A editora já está cadastrada'
    return JsonResponse(data)

class EditoraListView(PageInfoMixin, ModelFormMixin, ListView):
    model = Editora
    fields = '__all__'
...
class EditoraCreateView(PageInfoMixin, CreateView):
    model = Editora
    fields = '__all__'
...
class EditoraUpdateView(PageInfoMixin, UpdateView):
    model = Editora
    fields = '__all__'
    success_url = reverse_lazy('editora-list')
...
def loja_nome_registrado(request):
    ...
    if data['is_taken']:
        data['message'] = 'A loja já está cadastrada'
    return JsonResponse(data)

class LojaListView(PageInfoMixin, ModelFormMixin, ListView):
    model = Loja
    fields = '__all__'
...
class LojaCreateView(PageInfoMixin, CreateView):
    model = Loja
    fields = '__all__'
    ...
    def form_valid(self, form):
        if self.request.is_ajax():
            ...
            return JsonResponse({
                'obj': {
                    'nome': obj.nome,
                    'quantidade_de_clientes': obj.quantidade_de_clientes,
                }
            })
        else:
            return super().form_valid(form)
...
class LojaUpdateView(PageInfoMixin, UpdateView):
    model = Loja
    fields = '__all__'
    success_url = reverse_lazy('loja-list')
